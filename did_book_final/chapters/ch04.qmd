...
gen coef_sdid = .
gen se_sdid   = .
replace coef_twfe = 0 in 1

forvalues i = 1/21 {
    local row = `i' + 1
    replace coef_twfe = b_twfe[1, 59 + `i']           in `row'
    replace se_twfe   = sqrt(V_twfe[1, 59 + `i'])     in `row'
    replace coef_sdid = sdid_atts[1 + `i', 1]         in `row'
    replace se_sdid   = sdid_atts[1 + `i', 2]         in `row'
}
