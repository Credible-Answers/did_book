...
use "moser_voena_didtextbook.dta", clear
forvalues l = 1/18 {
    cap gen reltimeminus`l' = treatmentgroup * (year == 1918 - `l')
}
forvalues l = 1/21 {
    cap gen reltimeplus`l' = treatmentgroup * (year == 1918 + `l')
}
...
local leads ""
forvalues l = 18(-1)1 {
    local leads "`leads' reltimeminus`l'"
}
local lags ""
forvalues l = 1(1)21 {
    local lags "`lags' reltimeplus`l'"
}

coefplot, keep(reltimeminus* reltimeplus*) ///
    order(`leads' `lags'') ///
    vertical yline(0, lcolor(gs12) lpattern(solid)) xline(18.5, lpattern(dash)) ///
    title("TWFE Event-study estimates") ///
    xtitle("Relative time to year before TWEA") ytitle("Effect") ///
    scheme(s2mono) ///
    mcolor(gs4) ciopts(lcolor(gs4)) connect(l) lcolor(gs4) msymbol(O) msize(small) ///
    xlabel(1 "-18" 4 "-15" 7 "-12" 10 "-9" 13 "-6" 16 "-3" 18.5 "0" 21 "3" 24 "6" 27 "9" ///
           30 "12" 33 "15" 36 "18" 39 "21", noticks)
...
coefplot (m1, label("With Pre-Periods") mcolor(gs10) ciopts(lcolor(gs10)) connect(l) lcolor(gs10) msymbol(O) msize(small)) ///
         (m2, label("Without Pre-Periods") mcolor(gs4) ciopts(lcolor(gs4)) connect(l) lcolor(gs4) msymbol(O) msize(small)), ///
    keep(reltimeplus*) vertical ///
    ...
    scheme(s2mono) ///
    xlabel(1 "0" 4 "3" 7 "6" 10 "9" 13 "12" 16 "15" 19 "18" 21 "21", noticks)
